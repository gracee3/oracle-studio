...
- Chart import accepts only the pinned Astraeus calculation artifact. The
  schema-v3 chart workflow separately resolves an IANA local time to an exact
  UTC instant, invokes the pinned Astraeus Moshier adapter, and stores both the
  canonical artifact and an immutable application-owned input snapshot.
- Comparison calculation rebuilds derived layers from the exact stored
  calculation artifacts selected by the preset and stores a canonical
  Astraeus comparison artifact with those source calculation IDs.
...
`e5d295222018178c46fb882a302a57c810bf8bd1`; the metadata-only tarot fixture
