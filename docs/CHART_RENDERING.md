...
## Encrypted workspace integration

The Leptos workspace uses this renderer without exposing canonical vault
artifacts to the browser. After the native host validates and stores an
Astraeus comparison, it projects a versioned presentation DTO containing only
the immutable local-time/location headers and the selected scene population.
The WASM UI converts that DTO to `ChartScene` and calls `render_biwheel_svg`.
Both chart headers stay above the SVG, and static export downloads the exact
self-contained SVG generated by that call. The milestone intentionally does
not add the supplied aspect table.

## Current limitations

- The renderer remains stateless even when called from the encrypted
  workspace; the native host stores schema-v3 chart and comparison artifacts.
